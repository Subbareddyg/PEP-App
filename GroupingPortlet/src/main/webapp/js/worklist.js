   ...
   if(searchResult !="" && searchResult!= undefined && searchResult != 'undefined'){
	...
	if(upc.length>0){	
		$.get(url,{advSearchOperation:'searchSearch',
									 ...
									 searchResults: searchResults,},function(responseText) { 
					                	 alert("response text in search "+responseText);
				//$("#overlay_pageLoading").hide();
	              responseText = repalcePetTable(responseText);
...
// check if group is searched. If yes call different javascript function
						         ...
						         success: function(data){
									...
									$(json).each(function(i,val){
										...
										if(petLockedStatus){
											...
											document.getElementById("groupingType").value = groupType;
											$("#workListDisplayForm").submit();
											}						
