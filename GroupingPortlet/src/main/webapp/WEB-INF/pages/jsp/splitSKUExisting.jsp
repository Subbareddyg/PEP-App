<form name="existingComponentForm" id="existingComponentForm">
			...
			<table cellpadding="0" cellspacing="0" border="1" class="content-table border-simple colored-row tree-grid">
				<thead>
					<tr>
						<th width="10%"><label><input type="checkbox" <c:if test="${readonly =='yes'}"> disabled="disabled" </c:if> class="select-all" /> <fmt:message key="splitgroup.screen.level.component.selectAll" /></label></th>
	...
	{{_.each(data, function(row, key){ }}
		<tr>
			...
			<td class="text-center">
				<c:if test="${readonly =='yes'}">  
				<input type="radio" name="defaultColor" value="{{=row.colorCode}}_{{=row.size}}" {{=row.defaultColor == 'true' ? 'checked="checked"' : ''}} disabled="disabled" />
				</c:if>
				<c:if test="${readonly !='yes'}">  
				<input type="radio" name="defaultColor" class="trueDefult" value="{{=row.colorCode}}_{{=row.size}}" {{=row.defaultColor == 'true' ? 'checked="checked"' : ''}} />
				</c:if>
