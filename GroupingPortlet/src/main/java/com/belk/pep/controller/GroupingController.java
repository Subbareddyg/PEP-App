...
public class GroupingController {
	...
	private final static Logger LOGGER = Logger
			.getLogger(GroupingController.class.getName());
	...
	public ModelAndView splitColorRenderHandler(RenderRequest renderRequest,
			RenderResponse renderResponse) {
		// do some processing here
		LOGGER.info("GroupingControlle:splitRenderHandler:Enter");
		ModelAndView mv = new ModelAndView(
				GroupingConstants.GROUPING_CREATE_SPLIT_COLOR_GROUP);
		/** changes to display lan id - changed by Ramkumar - starts **/
		String sessionDataKey = (String) renderRequest.getPortletSession()
				.getAttribute("sessionDataKey");
		if (null != sessionDataKey) {
			String userID = sessionDataKey.split(GroupingConstants.USER_DATA
					+ renderRequest.getPortletSession().getId())[1];
	...
	public ModelAndView splitSKURenderHandler(RenderRequest renderRequest,
			RenderResponse renderResponse) {
		// do some processing here
		LOGGER.info("GroupingControlle:splitSKURenderHandler:Enter");
		ModelAndView mv = new ModelAndView(
				GroupingConstants.GROUPING_CREATE_SPLIT_SKU_GROUP);
		/** changes to display lan id - changed by Ramkumar - starts **/
		String sessionDataKey = (String) renderRequest.getPortletSession()
				.getAttribute("sessionDataKey");
		if (null != sessionDataKey) {
	...
	public ModelAndView handleRenderRequest(RenderRequest request,
		...
		if (null != sessionDataKey) {
			String userID = sessionDataKey.split(GroupingConstants.USER_DATA
					+ request.getPortletSession().getId())[1];
			LOGGER.info("display user ID" + userID);
	...
	public void handleEventRequest(EventRequest request, EventResponse response)
		...
		if (event.getName() != null) {
			LOGGER.info("GroupingController:handlingPagination:Enter 1 "
					+ event.getName());
			if (event.getName().equals(GroupingConstants.USER_DATA_OBJ)) {
				...
				if (null != custuser) {
					Common_BelkUser belkUser = (Common_BelkUser) custuser
							.getBelkUser();
					LOGGER.info("GroupingController:handlingPagination:Enter 2 ");
					if (custuser.getVpUser() != null) {
						if (null != custuser.getVpUser().getUserEmailAddress()) {
							LOGGER.info("This is from Event Email Id********************"
									+ custuser.getVpUser()
											.getUserEmailAddress());
							loggedInUser = custuser.getVpUser()
									.getUserEmailAddress();
						} else if (null != custuser.getRoleName()) {
							LOGGER.info("This is from Event Role name********************"
									+ custuser.getRoleName());
						} else if (null != custuser.getAccessRight()) {
							LOGGER.info("This is from Access********************"
									+ custuser.getAccessRight());
						}
					...
					if (null != belkUser) {
						...
						if (null != belkUser.getLanId()) {
							LOGGER.info("belkUser.getLanId() ******************"
									+ belkUser.getLanId());
							loggedInUser = belkUser.getLanId();
						}
					}

					String sessionDataKey = (GroupingConstants.USER_DATA
							+ request.getPortletSession().getId() + loggedInUser);
					String formSessionKey = request.getPortletSession().getId()
							+ loggedInUser;
					...
					request.getPortletSession().setAttribute("formSessionKey",
							formSessionKey);
					request.getPortletSession().setAttribute("sessionDataKey",
							sessionDataKey);
					request.getPortletSession().setAttribute(sessionDataKey,
							custuser);
				}
	...
	public ModelAndView handleSplitAttrSearchRequest(ResourceRequest request,
		...
		String formSessionKey = (String) request.getPortletSession()
				.getAttribute("formSessionKey");
		CreateGroupForm createGroupForm = (CreateGroupForm) request
				.getPortletSession().getAttribute(formSessionKey);
		String sessionDataKey = (String) request.getPortletSession()
				.getAttribute("sessionDataKey");
		UserData custuser = (UserData) request.getPortletSession()
				.getAttribute(sessionDataKey);

		LOGGER.debug(" handleSplitAttrSearchRequest formSessionKey --------------------------------------------------------------> "
				+ formSessionKey);
		LOGGER.debug(" handleSplitAttrSearchRequest resourceForm ----------------------------------------------------------------> "
				+ createGroupForm);
		LOGGER.debug(" handleSplitAttrSearchRequest sessionDataKey --------------------------------------------------------------> "
				+ sessionDataKey);
		LOGGER.info(" handleSplitAttrSearchRequest custuser ---------------------------------------------------------------------> "
				+ custuser);
		...
		LOGGER.info("in fetch split color attribute strFromPage--->"
				+ strFromPage);
		if (strFromPage.equals("SearchColor")) {
			LOGGER.debug("in fetch split color attribute-->");
			vendorStyleNo = request
					.getParameter(GroupingConstants.VENDOR_STYLE_NO);
			styleOrin = request.getParameter(GroupingConstants.STYLE_ORIN_NO);
			LOGGER.debug("Search Attribute: -- vendorStyleNo-->"
					+ vendorStyleNo + "  styleOrin-->" + styleOrin);
			// List<GroupAttributeForm> getSplitColorDetailsList =
			// groupingDelegate.getSplitColorDetails(vendorStyleNo, styleOrin);
			List<GroupAttributeForm> getSplitColorDetailsList = groupingService
					.getSplitColorDetails(vendorStyleNo, styleOrin);
			LOGGER.debug("Size of getSplitColorDetailsList-->"
					+ getSplitColorDetailsList.size());
			...
			LOGGER.debug("Size of updatedSplitColorDetailsList-->"
					+ updatedSplitColorDetailsList.size());
			if (getSplitColorDetailsList.size() == 0) {
				message = prop
						.getProperty(GroupingConstants.MESSAGE_SPLITGROUP_VALIDATION_NO_DATA); // "No record found!"
			} else if (updatedSplitColorDetailsList.size() == 0) {
				message = prop
						.getProperty(GroupingConstants.MESSAGE_SPLITGROUP_VALIDATION_INVALID_DATA);
			}
			/**
			...
			String totalRecords = String.valueOf(updatedSplitColorDetailsList
					.size());
			String defaultSortCol = GroupingConstants.STYLE_ORIN_NO; 
			String defaultSortOrder = GroupingConstants.SORT_ASC;
			JSONObject jsonObj = getSplitGrpJsonResponse(message, totalRecords,
					defaultSortCol, defaultSortOrder,
					updatedSplitColorDetailsList);
			...
			// groupingDelegate.getSplitSKUDetails(vendorStyleNo, styleOrin);
			List<GroupAttributeForm> getSplitSKUDetailsList = groupingService
					.getSplitSKUDetails(vendorStyleNo, styleOrin);
			LOGGER.debug("Size of getSplitSKUDetailsList-->"
					+ getSplitSKUDetailsList.size());

			/** Validate Split Attribute List **/
			List<GroupAttributeForm> updatedSplitSKUDetailsList = groupingService
					.validateSSGAttributeDetails(getSplitSKUDetailsList);
			LOGGER.debug("Size of updatedSplitSKUDetailsList-->"
					+ updatedSplitSKUDetailsList.size());
			...
			/** Code to generate response to display Search result in JSP **/
			String totalRecords = String.valueOf(updatedSplitSKUDetailsList
					.size());
			String defaultSortCol = GroupingConstants.STYLE_ORIN_NO; 
			String defaultSortOrder = GroupingConstants.SORT_ASC; 
			JSONObject jsonObj = getSplitGrpJsonResponse(message, totalRecords,
	 ...
	 * @author Cognizant
	 */
	public final JSONObject getSplitGrpJsonResponse(final String message,
			final String totalRecords, final String defaultSortCol,
		...
		try {
			for (int i = 0; i < selectedSplitAttributeList.size(); i++) {
				...
				isAlreadyInGroup = (null == isAlreadyInGroup ? "No" : ("N").equalsIgnoreCase(isAlreadyInGroup.trim()) ? "No"
	...
	public final ModelAndView handleCreateGroupForm(final ResourceRequest request,
			final ResourceResponse response) throws PEPServiceException,
			PEPPersistencyException {
		LOGGER.info("GroupingControlle:handleCreateGroupForm ResourceRequest:Enter------------>");

		String formSessionKey = (String) request.getPortletSession()
				.getAttribute("formSessionKey");
		LOGGER.info("handleRenderRequest formSessionKey  ----------------------------->"
				+ formSessionKey);
		String sessionDataKey = (String) request.getPortletSession()
				.getAttribute("sessionDataKey");
		LOGGER.info(" handleActionRequest sessionDataKey -----------------------------> "
				+ sessionDataKey);
		UserData custuser = (UserData) request.getPortletSession()
				.getAttribute(sessionDataKey);
		LOGGER.info(" handleActionRequest custuser -----------------------------------> "
				+ custuser);
		String pepUserId = custuser.getBelkUser().getLanId();
		LOGGER.info("This is from Reneder Internal User--------------------->"
				+ pepUserId);
		...
		String defaultSelectedAttr = request
				.getParameter(GroupingConstants.COMPONENT_DEFAULT_COLOR);
		String carsGroupingType = request
				.getParameter(GroupingConstants.CARS_GROUPING_TYPE);
		carsGroupingType = (null == carsGroupingType ? "" : carsGroupingType
				.trim());
		defaultSelectedAttr = (null == defaultSelectedAttr ? ""
				: defaultSelectedAttr.trim());
		String selectedItems = request
				.getParameter(GroupingConstants.COMPONENT_SELECTED_ITEMS);
		...
		LOGGER.debug("defaultSelectedAttr----------------->"
				+ defaultSelectedAttr);
		...
		if (null != groupType
				&& (GroupingConstants.GROUP_TYPE_SPLIT_COLOR).equals(groupType)
				&& null != selectedItems && null != selectedItemsArr) {
			// TODO
			/**
			 * Add Selected Attribute to List (selectedSplitAttributeList) for
			 * Split Color and Group
			 **/
			List<GroupAttributeForm> updatedSplitColorDetailsList = (List<GroupAttributeForm>) request
					.getPortletSession().getAttribute(
							GroupingConstants.SELECTED_ATTRIBUTE_LIST);
			LOGGER.debug("All Color Attribute List Size()-->"
					+ updatedSplitColorDetailsList.size());
			selectedSplitAttributeList = groupingService
					.getSelectedColorAttributeList(
							updatedSplitColorDetailsList, selectedItemsArr,
							defaultSelectedAttr);
		...
		if (null != groupType
				...
				&& null != selectedItems && null != selectedItemsArr) {
			List<GroupAttributeForm> updatedSplitSkuDetailsList = (List<GroupAttributeForm>) request
					.getPortletSession().getAttribute(
							GroupingConstants.SELECTED_ATTRIBUTE_LIST);
			LOGGER.debug("All SKU Attribute List Size()-->"
					+ updatedSplitSkuDetailsList.size());
			selectedSplitAttributeList = groupingService
					.getSelectedSKUAttributeList(updatedSplitSkuDetailsList,
		...
		CreateGroupForm createGroupFormRes = createGroup(createGroupForm,
				pepUserId);
		LOGGER.debug("After calling createGroup()-->getGroupCreationStatus: "
				+ createGroupFormRes.getGroupCreationStatus());
		LOGGER.debug("After calling createGroup()-->getGroupCretionMsg: "
				+ createGroupFormRes.getGroupCretionMsg());
		LOGGER.debug("After calling createGroup()-->GroupID: "
				+ createGroupFormRes.getGroupId());
		LOGGER.debug("After calling createGroup()-->GroupType: "
				+ createGroupFormRes.getGroupType());
		LOGGER.debug("After calling createGroup()-->GroupDesc: "
				+ createGroupFormRes.getGroupDesc());
		LOGGER.debug("After calling createGroup()-->getCarsGroupType: "
				+ createGroupFormRes.getCarsGroupType());

		request.getPortletSession().setAttribute(
				GroupingConstants.GROUP_DETAILS_FORM, createGroupFormRes);

		JSONObject jsonObj = new JSONObject();
		jsonObj.put(GroupingConstants.GROUP_ID, createGroupFormRes.getGroupId());
		jsonObj.put(GroupingConstants.GROUP_NAME,
				createGroupFormRes.getGroupName());
		jsonObj.put(GroupingConstants.GROUP_TYPE,
				createGroupFormRes.getGroupType());
		jsonObj.put(GroupingConstants.GROUP_DESC,
				createGroupFormRes.getGroupDesc());
		jsonObj.put(GroupingConstants.START_DATE,
				createGroupFormRes.getGroupLaunchDate());
		jsonObj.put(GroupingConstants.START_DATE,
				createGroupFormRes.getEndDate());
		jsonObj.put(GroupingConstants.GROUP_CREATION_MSG,
				createGroupFormRes.getGroupCretionMsg());
		jsonObj.put(GroupingConstants.GROUP_CREATION_STATUS_CODE,
				createGroupFormRes.getGroupCreationStatus());
		jsonObj.put(GroupingConstants.CARS_GROUPING_TYPE,
				createGroupFormRes.getCarsGroupType());
		...
		try {
			...
			LOGGER.error("GroupingControlle:handleCreateGroupForm ResourceRequest:Exception------------>"
					+ e.getMessage());
		}
	...
	public final ModelAndView createGroupSuccessRender(final RenderRequest renderRequest,
			final RenderResponse renderResponse) throws ParseException {
		// do some processing here
		...
		CreateGroupForm objCreateGroupForm = (CreateGroupForm) renderRequest
				.getPortletSession().getAttribute(
						GroupingConstants.GROUP_DETAILS_FORM);
		String groupTypeCode = (null == objCreateGroupForm.getGroupType() ? ""
				: objCreateGroupForm.getGroupType().trim());
		String groupTypeDesc = prop.getProperty(groupTypeCode);
		String groupStatusCode = (null == objCreateGroupForm.getGroupStatus() ? ""
				: objCreateGroupForm.getGroupStatus().trim());
		String groupStatusDesc = prop
				.getProperty(GroupingConstants.GROUP_STATUS_EXT
						+ groupStatusCode);
		String startDate = objCreateGroupForm.getGroupLaunchDate();
		if (null != startDate && !("").equals(startDate)) {
			startDateString = DateUtil.stringToStringMMddyyyy(startDate);
		}
		LOGGER.debug("createGroupSuccessRender.startDateString-->"
				+ startDateString);
		String endDate = objCreateGroupForm.getEndDate();
		if (null != endDate && !("").equals(endDate)) {
			endDateString = DateUtil.stringToStringMMddyyyy(endDate);
		}
		LOGGER.debug("createGroupSuccessRender.endDateString-->"
				+ endDateString);
		...
		LOGGER.debug("createGroupSuccessRender.objCreateGroupForm-->"
				+ objCreateGroupForm);
		LOGGER.debug("createGroupSuccessRender.objCreateGroupForm.getGroupId()-->"
				+ objCreateGroupForm.getGroupId());
		LOGGER.debug(groupTypeCode
				+ " createGroupSuccessRender.objCreateGroupForm.groupTypeDesc-->"
				+ groupTypeDesc);
		LOGGER.debug(groupStatusCode
				+ " createGroupSuccessRender.objCreateGroupForm.groupStatus-->"
				+ groupStatusDesc);
		ModelAndView mv = new ModelAndView(
				GroupingConstants.GROUPING_ADD_COMPONENT);
	 ...
	 * @author AFUPYB3
	 */
	private final CreateGroupForm createGroup(final CreateGroupForm createGroupForm,
			String updatedBy) {
		...
		try {
			...
			LOGGER.debug("json Object createGroup Group Name--> "
					+ jsonStyle.getString("groupName"));
		} catch (Exception e) {
			LOGGER.error("inside catch for createGroup()-->controller");
		}
		try {
			// createGroupFormRes =
			// groupingDelegate.saveGroupHeaderDetails(jsonStyle, updatedBy,
			// createGroupForm.getGroupAttributeFormList());
			createGroupFormRes = groupingService.saveGroupHeaderDetails(
					jsonStyle, updatedBy,
					createGroupForm.getGroupAttributeFormList());

			LOGGER.info("responseMsg_code Controller createGroup-->"
					+ createGroupFormRes.getGroupCretionMsg());
	 ...
	 * @author Cognizant
	 */
	public final JSONObject populateCreateGroupJson(final CreateGroupForm createGroupForm,
			final String updatedBy) {
		JSONObject jsonObj = new JSONObject();
		try {
			jsonObj.put(GroupingConstants.GROUP_NAME,
					createGroupForm.getGroupName());
			jsonObj.put(GroupingConstants.GROUP_TYPE,
					createGroupForm.getGroupType());
			jsonObj.put(GroupingConstants.GROUP_DESC,
					createGroupForm.getGroupDesc());
			jsonObj.put(GroupingConstants.START_DATE,
					createGroupForm.getGroupLaunchDate());
			jsonObj.put(GroupingConstants.END_DATE,
					createGroupForm.getEndDate());
			jsonObj.put(GroupingConstants.CREATED_BY, updatedBy);
			jsonObj.put(GroupingConstants.CARS_GROUPING_TYPE,
					createGroupForm.getCarsGroupType());
		} catch (JSONException e) {
			LOGGER.error("Exeception in parsing the jsonObj");
		}
		return jsonObj;
	...
	public final JSONObject searchGroupJsonObject(final GroupSearchForm groupSearchForm) {
		...
		jsonObjSearch.put(GroupingConstants.GROUP_ID_SEARCH,
				groupSearchForm.getGroupId());
		jsonObjSearch.put(GroupingConstants.GROUP_NAME_SEARCH,
				groupSearchForm.getGroupName());
		jsonObjSearch.put(GroupingConstants.DEPT_SEARCH,
				groupSearchForm.getDepts());
		jsonObjSearch.put(GroupingConstants.CLASS_SEARCH,
				groupSearchForm.getClasses());
		jsonObjSearch.put(GroupingConstants.VENDOR_SEARCH,
				groupSearchForm.getVendor());
		jsonObjSearch.put(GroupingConstants.SUPPLIER_SEARCH,
				groupSearchForm.getSupplierSiteId());
		jsonObjSearch.put(GroupingConstants.TOTAL_RECORD_COUNT,
				groupSearchForm.getTotalRecordCount());
		jsonObjSearch.put(GroupingConstants.RECORDS_PER_PAGE,
				groupSearchForm.getRecordsPerPage());
		jsonObjSearch.put(GroupingConstants.PAGE_NUMBER,
				groupSearchForm.getPageNumber());
		jsonObjSearch.put(GroupingConstants.SORTED_COLUMN,
				groupSearchForm.getSortColumn());
		jsonObjSearch.put(GroupingConstants.ASC_DESC_ORDER,
				groupSearchForm.getAscDescOrder());
		List<GroupForm> groupListMain = groupSearchForm.getGroupList();
		JSONArray mainArray = new JSONArray();
		for (Iterator<GroupForm> iterator = groupListMain.iterator(); iterator
				.hasNext();) {
			GroupForm groupForm = (GroupForm) iterator.next();
			JSONObject jsonObjMain = new JSONObject();
			jsonObjMain.put(GroupingConstants.GROUP_ID, groupForm.getGroupId());
			jsonObjMain.put(GroupingConstants.GROUP_NAME,
					groupForm.getGroupName());
			jsonObjMain.put(GroupingConstants.GROUP_TYPE,
					groupForm.getGroupType());
			jsonObjMain.put(GroupingConstants.GROUP_TYPE_CODE,
					groupForm.getGroupTypeCode());
			jsonObjMain.put(GroupingConstants.GROUP_CONTENT_STATUS,
					groupForm.getGroupContentStatus());
			jsonObjMain.put(GroupingConstants.GROUP_IMAGE_STATUS,
					groupForm.getGroupImageStatus());
			jsonObjMain.put(GroupingConstants.START_DATE,
					groupForm.getStartDate());
			jsonObjMain.put(GroupingConstants.END_DATE, groupForm.getEndDate());
			jsonObjMain.put(GroupingConstants.CHILD_GROUP,
					groupForm.getChildGroup());
			...
			if (groupListChild != null && groupListChild.size() > 0) {
				for (Iterator<GroupForm> iterator2 = groupListChild.iterator(); iterator2
					...
					JSONObject jsonObj = new JSONObject();
					jsonObj.put(GroupingConstants.GROUP_ID,
							groupFormChild.getGroupId());
					jsonObj.put(GroupingConstants.GROUP_NAME,
							groupFormChild.getGroupName());
					jsonObj.put(GroupingConstants.GROUP_TYPE,
							groupFormChild.getGroupType());
					jsonObj.put(GroupingConstants.GROUP_TYPE_CODE,
							groupFormChild.getGroupTypeCode());
					jsonObj.put(GroupingConstants.GROUP_CONTENT_STATUS,
							groupFormChild.getGroupContentStatus());
					jsonObj.put(GroupingConstants.GROUP_IMAGE_STATUS,
							groupFormChild.getGroupImageStatus());
					jsonObj.put(GroupingConstants.START_DATE,
							groupFormChild.getStartDate());
					jsonObj.put(GroupingConstants.END_DATE,
							groupFormChild.getEndDate());
					jsonObj.put(GroupingConstants.CHILD_GROUP,
							groupFormChild.getChildGroup());
	 ...
	 */
	@ResourceMapping
	public final ModelAndView handleResourceRequest(final ResourceRequest request,
			final ResourceResponse response) throws Exception {

		LOGGER.info("Entering handleResourceRequest() in GroupingController class.");
		String action = GroupingUtil.checkNull(request
				.getParameter("resourceType"));
		ModelAndView modelAndView = null;
		LOGGER.debug("Action is -- " + action);
		if (action.equals("searchGroup")) {
			GroupSearchForm searchForm = new GroupSearchForm();
			searchForm.setVendor(GroupingUtil.checkNull(request
					.getParameter("vendor")));
			searchForm.setOrinNumber(GroupingUtil.checkNull(request
					.getParameter("orinNumber")));
			searchForm.setGroupId(GroupingUtil.checkNull(request
					.getParameter("groupId")));
			searchForm.setGroupName(GroupingUtil.checkNull(request
					.getParameter("groupName")));
			searchForm.setDepts(GroupingUtil.checkNull(request
					.getParameter("departments")));
			searchForm.setClasses(GroupingUtil.checkNull(request
					.getParameter("classes")));
			searchForm.setSupplierSiteId(GroupingUtil.checkNull(request
					.getParameter("supplierId")));
			if (request.getParameter("totalRecordsCount") != null) {
				searchForm.setTotalRecordCount(Integer.parseInt(request
						.getParameter("totalRecordsCount")));
			} else {
				searchForm.setTotalRecordCount(0);
			}
			if (request.getParameter("recordsPerPage") != null) {
				searchForm.setRecordsPerPage(Integer.parseInt(request
						.getParameter("recordsPerPage")));
			} else {
				searchForm.setRecordsPerPage(0);
			}
			if (request.getParameter("pageNumber") != null) {
				searchForm.setPageNumber(Integer.parseInt(request
						.getParameter("pageNumber")));
			} else {
				searchForm.setPageNumber(1);
			}
			searchForm.setSortColumn(GroupingUtil.checkNull(request
					.getParameter("sortedColumn")));
			searchForm.setAscDescOrder(GroupingUtil.checkNull(request
					.getParameter("ascDescOrder")));
			LOGGER.debug("Search values from screen -- \nGROUP ID: "
					+ searchForm.getGroupId() + "\nGROUP NAME: "
					+ searchForm.getGroupName() + "\nORIN#: "
					+ searchForm.getOrinNumber() + "\nVENDOR: "
					+ searchForm.getVendor() + "\nSUPPILER ID: "
					+ searchForm.getSupplierSiteId() + "\nDEPARTMENTS: "
					+ searchForm.getDepts() + "\nCLASSES: "
			...
			if (searchForm.getGroupId() != null
					&& !searchForm.getGroupId().trim().equals("")) {
				searchForm.setTotalRecordCount(groupingService
						.groupSearchCount(searchForm)
						+ searchForm.getParentCount());
			} else {
				searchForm.setTotalRecordCount(groupingService
						.groupSearchCount(searchForm));
			}
			...
			try {
				...
				LOGGER.error("GroupingController:Group Search ResourceRequest:Exception------------>"
						+ e.getMessage());
			}
		}
		if (action.equals("searchDept")) {
			ArrayList<DepartmentDetails> deptList = groupingService
					.getDeptDetailsByDepNoFromADSE();
			JSONObject jsonObject = departmentListJsonObject(deptList);
			try {
				response.getWriter().write(jsonObject.toString());
			} catch (IOException e) {
				LOGGER.error("GroupingController:Dept Search ResourceRequest:Exception------------>"
						+ e.getMessage());
			}
		}
		if (action.equals("searchClass")) {
			String depts = GroupingUtil
					.checkNull(request.getParameter("depts"));
			List<ClassDetails> classList = groupingService
					.getClassDetailsByDepNos(depts);
		...
		if (action.equals("deleteGroup")) {
			...
			LOGGER.info("handleDeleteGroup formSessionKey  ----------------------------->"
					+ formSessionKey);
			...
			try {
				String groupId = request
						.getParameter(GroupingConstants.GROUP_ID);
				String groupType = request
				...
				json.put(GroupingConstants.DELETE_STATUS_MESSAGE,
						"#" + GroupingUtil.checkNull(groupId) + " "
								+ GroupingUtil.checkNull(responseMesage));
				json.put(GroupingConstants.DELETE_STATUS, status);
				try {
					response.getWriter().write(json.toString());
				} catch (IOException e) {
					LOGGER.error("GroupingController:Group delete ResourceRequest:Exception------------>"
							+ e.getMessage());
				}
			} catch (Exception e) {
	 ...
	 *         Added By: Cognizant
	 */
	public final JSONObject departmentListJsonObject(
			final ArrayList<DepartmentDetails> departmentList) {
		...
		for (Iterator<DepartmentDetails> iterator = departmentList.iterator(); iterator
				.hasNext();) {
	...
	public final JSONObject classListJsonObject(final List<ClassDetails> classList) {
		...
		for (Iterator<ClassDetails> iterator = classList.iterator(); iterator
				.hasNext();) {
	...
	public final ModelAndView handleDeleteGroup(final ResourceRequest request,
		...
		String sessionDataKey = (String) request.getPortletSession()
				.getAttribute("sessionDataKey");
		LOGGER.info(" handleDeleteGroup sessionDataKey -----------------------------> "
				+ sessionDataKey);
		UserData custuser = (UserData) request.getPortletSession()
				.getAttribute(sessionDataKey);
		LOGGER.info(" handleDeleteGroup custuser -----------------------------------> "
				+ custuser);
		String pepUserId = custuser.getBelkUser().getLanId();
		LOGGER.info("This is from Reneder Internal User--------------------->"
				+ pepUserId);
		...
		try {
			...
			String responseMesage = "";
			if (null != groupId) {
				responseMesage = groupingService.deleteGroup(groupId,
						groupType, pepUserId);
			}
			JSONObject json = new JSONObject();
			...
			LOGGER.error("handleDeleteGroup ResourceRequest:Exception------------>"
					+ e.getMessage());
		}
	...
	public final void getExistingGrpDetails(final ActionRequest request,
			final ActionResponse response) throws PEPFetchException, Exception {
		// do some processing here
		LOGGER.info("GroupingControlle:getExistingGrpDetails:Enter-->");
		String groupType = GroupingUtil.checkNull(request
				.getParameter(GroupingConstants.GROUP_TYPE));
		String groupId = GroupingUtil.checkNull(request
				.getParameter(GroupingConstants.GROUP_ID));
		LOGGER.debug("getExistingGrpDetails.groupType---------->" + groupType);
		LOGGER.debug("getExistingGrpDetails.groupId---------->" + groupId);

		request.getPortletSession().setAttribute(GroupingConstants.GROUP_TYPE,
				groupType);
		request.getPortletSession().setAttribute(GroupingConstants.GROUP_ID,
				groupId);

		response.setRenderParameter("existingGrpDetailsRender",
				"existingGrpDetailsRender");
	...
	public final ModelAndView existingGrpDetailsRender(final RenderRequest request,
		...
		String groupType = (String) request.getPortletSession().getAttribute(
				GroupingConstants.GROUP_TYPE);
		String groupId = (String) request.getPortletSession().getAttribute(
				GroupingConstants.GROUP_ID);
		...
		CreateGroupForm objCreateGroupForm = groupingService
				.getExistingGrpDetails(groupId);
		...
		if(LOGGER.isDebugEnabled()){
			LOGGER.debug("getExistingGrpDetails.startDateString-->"
					+ startDateString);
		}
		String endDate = objCreateGroupForm.getEndDate();
		if (null != endDate && !("").equals(endDate)) {
		...
		if(LOGGER.isDebugEnabled()){
			...
			LOGGER.debug(groupTypeCode
					+ " getExistingGrpDetails.objCreateGroupForm.groupTypeDesc-->" + groupTypeDesc);
			LOGGER.debug(groupStatusCode
					+ " getExistingGrpDetails.objCreateGroupForm.groupStatus-->" + groupStatusDesc);
		}
		modelAndView.addObject(GroupingConstants.GROUP_DETAILS_FORM,
				objCreateGroupForm);
	...
	public final ModelAndView getExistGrpComponentResource(final ResourceRequest request,
			final ResourceResponse response) throws PEPServiceException,
			PEPPersistencyException {
		LOGGER.info("GroupingControlle:getExistGrpComponentResource ResourceRequest:Enter------------>");
		String message = "";
		ModelAndView modelAndView = null;
		List<GroupAttributeForm> existComponentDetails = null;
		...
		if (existComponentDetails.size() == 0) {
			message = prop
					.getProperty(GroupingConstants.MESSAGE_SPLITGROUP_VALIDATION_NO_DATA);
		} 
		String totalRecords = String.valueOf(existComponentDetails.size());
		String defaultSortCol = GroupingConstants.STYLE_ORIN_NO; 
		String defaultSortOrder = GroupingConstants.SORT_ASC;
		JSONObject jsonObj = getSplitGrpJsonResponse(message, totalRecords,
				defaultSortCol, defaultSortOrder, existComponentDetails);
		if(LOGGER.isDebugEnabled()){
		...
		try {
			...
			LOGGER.error("GroupingControlle:getExistGrpComponentResource:Exception------------>"
					+ e.getMessage());
		}
