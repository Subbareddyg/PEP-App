...
public class XqueryConstants {
	...
	/** The Constant LOGGER. */
	private static final Logger LOGGER = Logger.getLogger(XqueryConstants.class
			.getName());
	...
	public final String getSplitColorDetails(final String vendorStyleNo) {
		...
		if (vendorStyleNo != null) {
			getSplitColorDetailsQuery = getSplitColorDetailsQuery
					+ " AND item.PRIMARYSUPPLIERVPN = :styleIdSql";
		} else {
			getSplitColorDetailsQuery = getSplitColorDetailsQuery
					+ " AND ITEM.MDMID = :mdmidSql";
		}
	...
	public final String getSplitSKUDetails(final String vendorStyleNo) {
		String getSplitSKUDetailsQuery = "   Select                                                                      "
				...
				+ "      AND PET.MDMID=ITEM.MDMID                                                                        ";
		if (vendorStyleNo != null) {
			getSplitSKUDetailsQuery = getSplitSKUDetailsQuery
					+ " AND item.PRIMARYSUPPLIERVPN = :styleIdSql";
		} else {
			getSplitSKUDetailsQuery = getSplitSKUDetailsQuery
					+ " AND ITEM.MDMID = :mdmidSql";
	 ...
	 *         Added By: Cognizant
	 */
	public final String getGroupDetailsQuery(
		...
		getGroupDetailsQuery
				.append(" SELECT GRP.MDMID GROUP_ID, GRP.GROUP_NAME, GRP.ENTRY_TYPE GROUP_TYPE," 
		...
		if((groupSearchForm.getVendor() != null && !groupSearchForm
				.getVendor().trim().equals(""))
				|| (groupSearchForm.getDepts() != null && !groupSearchForm
						.getDepts().trim().equals(""))
				|| (groupSearchForm.getClasses() != null && !groupSearchForm
						.getClasses().trim().equals(""))
				|| (groupSearchForm.getOrinNumber() != null && !groupSearchForm
						.getOrinNumber().trim().equals(""))
				|| (groupSearchForm.getSupplierSiteId() != null && !groupSearchForm
						.getSupplierSiteId().trim().equals("")))
		{
			getGroupDetailsQuery
				.append(" ,ADSE_GROUP_CHILD_MAPPING GCM, ADSE_ITEM_CATALOG AIC ");
		}
		...
		if (groupSearchForm.getGroupName() != null
				&& !groupSearchForm.getGroupName().trim().equals("")) {
			getGroupDetailsQuery.append(" AND UPPER(GROUP_NAME) LIKE '%");
			getGroupDetailsQuery.append(groupSearchForm.getGroupName()
					.toUpperCase());
			getGroupDetailsQuery.append("%'");
		}
		if ((groupSearchForm.getVendor() != null && !groupSearchForm
			...
			getGroupDetailsQuery
					.append(" AND GCM.COMPONENT_STYLE_ID = AIC.MDMID ");
			getGroupDetailsQuery
					.append(" AND AIC.DELETED_FLAG = 'false' ");
			getGroupDetailsQuery
					.append(" AND AIC.ENTRY_TYPE = 'Style' ");
			if (groupSearchForm.getDepts() != null
					&& !groupSearchForm.getDepts().trim().equals("")) {
				getGroupDetailsQuery.append(" AND AIC.DEPT_ID IN (");
				getGroupDetailsQuery
						.append(getNumbersInCorrectFormat(groupSearchForm
								.getDepts()));
				getGroupDetailsQuery.append(")");
			}
			if (groupSearchForm.getClasses() != null
					&& !groupSearchForm.getClasses().trim().equals("")) {
			...
			if (groupSearchForm.getOrinNumber() != null
					&& !groupSearchForm.getOrinNumber().trim().equals("")) {
				getGroupDetailsQuery.append(" AND AIC.MDMID = '");
				getGroupDetailsQuery.append(groupSearchForm.getOrinNumber());
				getGroupDetailsQuery.append("'");
			}
			if (groupSearchForm.getSupplierSiteId() != null
					&& !groupSearchForm.getSupplierSiteId().trim().equals("")) {
				getGroupDetailsQuery.append(" AND AIC.PRIMARY_SUPPLIER_ID = '");
				getGroupDetailsQuery
						.append(groupSearchForm.getSupplierSiteId());
				getGroupDetailsQuery.append("'");
			}
			if (groupSearchForm.getVendor() != null
					&& !groupSearchForm.getVendor().trim().equals("")) {
		...
		String sortOrder = "";
		if (groupSearchForm.getAscDescOrder() != null
				|| !groupSearchForm.getAscDescOrder().trim().equals("")) {
			sortOrder = groupSearchForm.getAscDescOrder();
		}
		if (groupSearchForm.getSortColumn() == null
				|| groupSearchForm.getSortColumn().trim().equals("")) {
			getGroupDetailsQuery.append(" ORDER BY GRP.ENTRY_TYPE, GRP.MDMID "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupId")) {
			getGroupDetailsQuery.append(" ORDER BY GRP.MDMID " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupName")) {
			getGroupDetailsQuery.append(" ORDER BY GROUP_NAME " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("startDate")) {
			getGroupDetailsQuery
					.append(" ORDER BY GRP.START_DATE " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("endDate")) {
			getGroupDetailsQuery.append(" ORDER BY GRP.END_DATE " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupType")) {
			getGroupDetailsQuery
					.append(" ORDER BY GRP.ENTRY_TYPE " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("imageStatus")) {
			getGroupDetailsQuery.append(" ORDER BY IMAGE_STATE.THEVALUE "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("contentStatus")) {
			getGroupDetailsQuery.append(" ORDER BY CONTENT_STATE.THEVALUE "
					+ sortOrder);
		}
	 ...
	 *         Added By: Cognizant
	 */
	public final String getGroupDetailsCountQuery(
			final GroupSearchForm groupSearchForm) {
		...
		getGroupDetailsCountQuery
				.append(" SELECT COUNT(*) TOTAL_COUNT FROM ADSE_GROUP_CATALOG GRP INNER JOIN ");
		getGroupDetailsCountQuery
				.append(" ADSE_REFERENCE_DATA CONTENT_STATE ON GROUP_CONTENT_STATUS_CODE = CONTENT_STATE.MDMID ");
		getGroupDetailsCountQuery
				.append(" AND CONTENT_STATE.ENTRY_TYPE = 'ContentState_Lookup' ");
		...
		if (groupSearchForm.getGroupName() != null
			...
			getGroupDetailsCountQuery.append(groupSearchForm.getGroupName()
					.toUpperCase());
			getGroupDetailsCountQuery.append("%'");
		}
		if ((groupSearchForm.getVendor() != null && !groupSearchForm
			...
			if (groupSearchForm.getOrinNumber() != null
				...
				getGroupDetailsCountQuery.append(groupSearchForm
						.getOrinNumber());
				getGroupDetailsCountQuery.append("'");
			}
			if (groupSearchForm.getSupplierSiteId() != null
					&& !groupSearchForm.getSupplierSiteId().trim().equals("")) {
				getGroupDetailsCountQuery
						.append(" AND AIC.PRIMARY_SUPPLIER_ID = '");
				getGroupDetailsCountQuery.append(groupSearchForm
						.getSupplierSiteId());
				getGroupDetailsCountQuery.append("'");
			}
			if (groupSearchForm.getVendor() != null
					&& !groupSearchForm.getVendor().trim().equals("")) {
				getGroupDetailsCountQuery
						.append(" AND AIC.PRIMARYSUPPLIERVPN = '");
	...
	public final String getGroupDetailsQueryParent(
		...
		getGroupDetailsQueryParent
				.append(" SELECT AGC.MDMID GROUP_ID, AGC.GROUP_NAME GROUP_NAME, AGC.ENTRY_TYPE GROUP_TYPE,"
		...
		getGroupDetailsQueryParent
				.append(" FROM ADSE_GROUP_CATALOG AGC INNER JOIN ");
		getGroupDetailsQueryParent
				.append(" ADSE_REFERENCE_DATA CONTENT_STATE ON GROUP_CONTENT_STATUS_CODE = CONTENT_STATE.MDMID ");
		getGroupDetailsQueryParent
				.append(" AND CONTENT_STATE.ENTRY_TYPE = 'ContentState_Lookup' ");
		...
		if (groupIdsList.size() > 0) {
			int count = 0;
			for (Iterator<String> iterator = groupIdsList.iterator(); iterator
					.hasNext();) {
				final String strGroupIds = (String) iterator.next();
				if (count == 0) {
					getGroupDetailsQueryParent
							.append(" AND (GCM.COMPONENT_GROUPING_ID IN ("
									+ strGroupIds + ")");
				} else {
					getGroupDetailsQueryParent
							.append(" OR GCM.COMPONENT_GROUPING_ID IN ("
									+ strGroupIds + ")");
				}
				count++;
			}
		...
		if (groupSearchForm.getSortColumn() == null
				|| groupSearchForm.getSortColumn().trim().equals("")) {
			getGroupDetailsQueryParent
					.append(" ORDER BY AGC.ENTRY_TYPE, AGC.MDMID " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupId")) {
			getGroupDetailsQueryParent.append(" ORDER BY AGC.MDMID "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupName")) {
			getGroupDetailsQueryParent.append(" ORDER BY AGC.GROUP_NAME "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("startDate")) {
			getGroupDetailsQueryParent.append(" ORDER BY START_DATE "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("endDate")) {
			getGroupDetailsQueryParent
					.append(" ORDER BY END_DATE " + sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("groupType")) {
			getGroupDetailsQueryParent.append(" ORDER BY AGC.ENTRY_TYPE "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("imageStatus")) {
			getGroupDetailsQueryParent.append(" ORDER BY IMAGE_STATE.THEVALUE "
					+ sortOrder);
		} else if (groupSearchForm.getSortColumn() != null
				&& groupSearchForm.getSortColumn().equals("contentStatus")) {
			getGroupDetailsQueryParent
					.append(" ORDER BY CONTENT_STATE.THEVALUE " + sortOrder);
		}
	 ...
	 *         Added By: Cognizant
	 */
	public final String getGroupDetailsCountQueryParent(
			final List<GroupSearchDTO> groupSearchDTOList,
			final GroupSearchForm groupSearchForm) {
		...
		final StringBuffer getGroupDetailsQueryParent = new StringBuffer();
		getGroupDetailsQueryParent
				.append(" SELECT COUNT(*) TOTAL_COUNT_PARENT ");
		getGroupDetailsQueryParent
				.append(" FROM ADSE_GROUP_CATALOG AGC INNER JOIN ");
		getGroupDetailsQueryParent
				.append(" ADSE_REFERENCE_DATA CONTENT_STATE ON GROUP_CONTENT_STATUS_CODE = CONTENT_STATE.MDMID ");
	 ...
	 *         Added By: Cognizant
	 */
	public final List<String> getCommaSeparatedValues(
			final List<GroupSearchDTO> groupSearchDTOList) {
		...
		if (groupSearchDTOList.size() > 0) {
			...
			for (GroupSearchDTO groupSearchDTO : groupSearchDTOList) {
				...
				if (count > 999) {
					...
					listGroupIds.add(strGroupIdList.substring(0,
							strGroupIdList.length() - 1));
					count = 0;
	...
	public final String getLikeDepartmentDetailsForString() {
		...
		final String getAllDepartmentQuery = " select DeptId DEPTID, DeptName DEPTNAME"
	...
	public final String getClassDetailsUsingDeptnumbers(final String deptids) {
		...
		queryfragment
				.append("  SELECT CLS_NAME.ClsId CLASS_ID, CLS_NAME.ClsName CLASS_NAME, CLS_NAME.Removal_Flag  ");
		queryfragment
	...
	public final String getExistSplitColorDetails() {
		String getExistSplitColorDetails = "	SELECT                                                                           "
				...
				+ "	AND DETAIL.MDMID=?  ";  // /*StyleColor MDMID*/
	...
	public final String getExistSplitSkuDetails() {
		String getExistSplitSkuDetails = "	SELECT                                                "
			...
			/*+"	  OR ITEM.MDMID = DETAIL.COMPONENT_STYLE_ID)                          "*/ // To fetch Style data for that Color for Product name
			+"	AND DETAIL.MDMID=?                                             "; // StyleSKU MDMID
