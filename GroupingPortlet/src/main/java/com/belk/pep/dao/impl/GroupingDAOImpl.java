...
public class GroupingDAOImpl implements GroupingDAO {
	 ...
	 */
	public CreateGroupDTO getGroupHeaderDetails(final String groupId)
			throws PEPFetchException {
		...
		try {
			...
			if (rows != null) {
				...
				for (final Object row : rows) {
					...
					final String groupName = rowMap.get("GROUP_NAME") != null ? rowMap
							.get("GROUP_NAME").toString() : "";
					...
					final String startDate = rowMap.get("EFFECTIVE_START_DATE") != null ? rowMap
							.get("EFFECTIVE_START_DATE").toString() : "";
					final String endDate = rowMap.get("EFFECTIVE_END_DATE") != null ? rowMap
							.get("EFFECTIVE_END_DATE").toString() : "";
					final String groupStatus = rowMap
							.get("GROUP_OVERALL_STATUS_CODE") != null ? rowMap
							.get("GROUP_OVERALL_STATUS_CODE").toString() : "";
					final String groupType = rowMap.get("GROUP_TYPE") != null ? rowMap
							.get("GROUP_TYPE").toString() : "";
					final String carsGroupType = rowMap.get("CARS_GROUP_TYPE") != null ? rowMap
							.get("CARS_GROUP_TYPE").toString() : "";

					LOGGER.debug("getGroupHeaderDetails.groupName-->"
							+ groupName);
					LOGGER.debug("getGroupHeaderDetails.startDate-->"
							+ startDate);
					LOGGER.debug("getGroupHeaderDetails.groupDesc-->"
							+ groupDesc);
					LOGGER.debug("getGroupHeaderDetails.groupType-->"
							+ groupType);
					LOGGER.debug("getGroupHeaderDetails.carsGroupType-->"
							+ carsGroupType);
	 ...
	 */
	public List<GroupAttributeForm> getSplitColorDetails(
			final String vendorStyleNumber, final String styleOrin) {
		...
		try {
			session = sessionFactory.openSession();
			vendorStyleNo = null == vendorStyleNo ? null : vendorStyleNo.trim()
					.equals("") ? null : vendorStyleNo.trim();
			// Hibernate provides a createSQLQuery method to let you call your
			// native SQL statement directly.
			final Query query = session.createSQLQuery(xqueryConstants
					.getSplitColorDetails(vendorStyleNo));
			...
			if (rows != null) {
				...
				for (final Object row : rows) {
					...
					final String mdmid = rowMap.get("MDMID") != null ? rowMap
							.get("MDMID").toString() : "";
					// String parentMdmid=rowMap.get("PARENT_MDMID") != null ?
					// rowMap.get("PARENT_MDMID").toString() : "";
					final String styleNo = rowMap.get("PRIMARYSUPPLIERVPN") != null ? rowMap
							.get("PRIMARYSUPPLIERVPN").toString() : "";
					final String productName = rowMap.get("PRODUCT_NAME") != null ? rowMap
							.get("PRODUCT_NAME").toString() : "";
					final String colorCode = rowMap.get("COLOR_CODE") != null ? rowMap
							.get("COLOR_CODE").toString() : "";
					final String colorDesc = rowMap.get("COLOR_DESC") != null ? rowMap
							.get("COLOR_DESC").toString() : "";
					final String isAlreadyInGroup = rowMap
							.get("ALREADY_IN_GROUP") != null ? rowMap.get(
							"ALREADY_IN_GROUP").toString() : "N";
					final String petStatus = rowMap.get("PET_STATE") != null ? rowMap
							.get("PET_STATE").toString() : "";
					final String entryType = rowMap.get("ENTRY_TYPE") != null ? rowMap
							.get("ENTRY_TYPE").toString() : "";
	 ...
	 * @return List<GroupAttributeForm>
	 */
	public List<GroupAttributeForm> getSplitSKUDetails(
			final String vendorStyleNumber, final String styleOrin) {
		...
		try {
			...
			// native SQL statement directly.
			final Query query = session.createSQLQuery(xqueryConstants
					.getSplitSKUDetails(vendorStyleNo));
			...
			if (rows != null) {
				...
				for (final Object row : rows) {
					...
					final String styleNo = rowMap.get("PRIMARYSUPPLIERVPN") != null ? rowMap
							.get("PRIMARYSUPPLIERVPN").toString() : "";
					final String productName = rowMap.get("NAME") != null ? rowMap
							.get("NAME").toString() : "";
					final String colorCode = rowMap.get("COLOR_CODE") != null ? rowMap
							.get("COLOR_CODE").toString() : "";
					final String colorDesc = rowMap.get("COLOR_NAME") != null ? rowMap
							.get("COLOR_NAME").toString() : "";
					final String sizeDesc = rowMap.get("SIZEDESC") != null ? rowMap
							.get("SIZEDESC").toString() : "";
					final String isAlreadyInGroup = rowMap
							.get("ALREADY_IN_GROUP") != null ? rowMap.get(
							"ALREADY_IN_GROUP").toString() : "N";
					final String petStatus = rowMap.get("PET_STATE") != null ? rowMap
	 ...
	 */
	@Override
	public List<GroupSearchDTO> groupSearch(
		...
		try {
			session = sessionFactory.openSession();
			final Query query = session.createSQLQuery(xqueryConstants
					.getGroupDetailsQuery(groupSearchForm));
			query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
			if (groupSearchForm.getGroupId() == null
					|| groupSearchForm.getGroupId().trim().equals("")) {
				query.setFirstResult((groupSearchForm.getPageNumber() - 1)
			...
			if (rows != null) {
				for (final Object rowObj : rows) {
					...
					groupSearchDTO.setGroupId(row.get("GROUP_ID") == null ? ""
							: row.get("GROUP_ID").toString());
					groupSearchDTO
							.setGroupName(row.get("GROUP_NAME") == null ? ""
									: row.get("GROUP_NAME").toString());
					groupSearchDTO
							.setGroupType(row.get("GROUP_TYPE") == null ? ""
									: row.get("GROUP_TYPE").toString());
					groupSearchDTO.setGroupContentStatus(row
							.get("CONTENT_STATE") == null ? "" : row.get(
							"CONTENT_STATE").toString());
					groupSearchDTO
							.setStartDate(row.get("START_DATE") == null ? ""
									: row.get("START_DATE").toString());
					groupSearchDTO.setEndDate(row.get("END_DATE") == null ? ""
							: row.get("END_DATE").toString());
					groupSearchDTO
							.setGroupImageStatus(row.get("IMAGE_STATE") == null ? ""
									: row.get("IMAGE_STATE").toString());
					groupSearchDTO.setChildGroup(row.get("CHILD_GROUP") ==
							null? "" : row.get("CHILD_GROUP").toString());

					LOGGER.debug("Grouping Attribute Values -- \nGROUP ID: "
							+ groupSearchDTO.getGroupId() + "\nGROUP NAME: "
							+ groupSearchDTO.getGroupName() + "\nGROUP TYPE: "
							+ groupSearchDTO.getGroupType()
							+ "\nGROUP CONTENT STATUS: "
							+ groupSearchDTO.getGroupContentStatus()
							+ "\nGROUP IMAGE STATUS: "
	...
	public int groupSearchCount(final GroupSearchForm groupSearchForm)
		...
		try {
			session = sessionFactory.openSession();
			final Query query = session.createSQLQuery(xqueryConstants
					.getGroupDetailsCountQuery(groupSearchForm));
			query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
			rows = query.list();
			...
			if (rows != null) {
				for (final Object rowObj : rows) {
					final Map row = (Map) rowObj;
					rowCount = (BigDecimal) (row.get("TOTAL_COUNT") == null ? ""
							: row.get("TOTAL_COUNT"));
	 ...
	 */
	@Override
	public List<GroupSearchDTO> groupSearchParent(
			final List<GroupSearchDTO> groupSearchDTOList,
			final GroupSearchForm groupSearchForm) throws PEPServiceException,
			PEPPersistencyException {
		...
		try {
			session = sessionFactory.openSession();
			final Query query = session.createSQLQuery(xqueryConstants
					.getGroupDetailsQueryParent(groupSearchDTOList,
							groupSearchForm));
			query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
			if (groupSearchForm.getPageNumber() == 1) {
				query.setFirstResult((groupSearchForm.getPageNumber() - 1)
						* groupSearchForm.getRecordsPerPage() - 1);
				query.setMaxResults(groupSearchForm.getRecordsPerPage());
			} else {
				query.setFirstResult(((groupSearchForm.getPageNumber() - 1) * groupSearchForm
						.getRecordsPerPage()) - 1);
				query.setMaxResults(groupSearchForm.getRecordsPerPage());
			}
			rows = query.list();
			...
			if (rows != null) {
				for (final Object rowObj : rows) {
					...
					groupSearchDTO.setEndDate(row.get("END_DATE") == null ? ""
							: row.get("END_DATE").toString());
					groupSearchDTO.setComponentGroupId(row
							.get("COMPONENT_GROUPING_ID") == null ? "" : row
							.get("COMPONENT_GROUPING_ID").toString());
					groupSearchDTO
							.setGroupImageStatus(row.get("IMAGE_STATE") == null ? ""
									: row.get("IMAGE_STATE").toString());
					groupSearchDTO.setChildGroup(row.get("CHILD_GROUP") ==
					...
					LOGGER.debug("Grouping Attribute Values -- \nGROUP ID: "
							...
							+ "\nEND DATE: " + groupSearchDTO.getEndDate()
							+ "\nCOMPONENT GROUP ID: "
							+ groupSearchDTO.getComponentGroupId());
	 ...
	 */
	@Override
	public int groupSearchParentCount(
			final List<GroupSearchDTO> groupSearchDTOList,
			final GroupSearchForm groupSearchForm) throws PEPServiceException,
			PEPPersistencyException {
		...
		try {
			session = sessionFactory.openSession();
			final Query query = session.createSQLQuery(xqueryConstants
					.getGroupDetailsCountQueryParent(groupSearchDTOList,
							groupSearchForm));
			query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
			rows = query.list();
			if (rows != null) {
				for (final Object rowObj : rows) {
					final Map row = (Map) rowObj;
					rowCount = (BigDecimal) (row.get("TOTAL_COUNT_PARENT") == null ? ""
							: row.get("TOTAL_COUNT_PARENT"));
	 ...
	 */
	@Override
	public ArrayList<DepartmentDetails> getDeptDetailsByDepNoFromADSE()
			throws PEPPersistencyException {
		...
		try {
			session = sessionFactory.openSession();
			final Query query1 = session.createSQLQuery(xqueryConstants
					.getLikeDepartmentDetailsForString());
			...
			if (query1 != null) {
				...
				if (rows1 != null && rows1.size() > 0) {
					for (final Object row : rows1) {
						...
						departmentDetails
								.setId(rowObj.get("DEPTID") == null ? ""
										: rowObj.get("DEPTID").toString());
						departmentDetails
								.setDesc(rowObj.get("DEPTNAME") == null ? ""
										: rowObj.get("DEPTNAME").toString());
	 ...
	 */
	@Override
	public List<ClassDetails> getClassDetailsByDepNos(
		...
		try {
			session = sessionFactory.openSession();
			final Query query = session.createSQLQuery(xqueryConstants
					.getClassDetailsUsingDeptnumbers(departmentNumbers));
			query.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
			final List<Object> rows = query.list();
			for (final Object row : rows) {
				final Map rowObj = (Map) row;
				final ClassDetails classDetails = new ClassDetails();
				classDetails.setId(rowObj.get("CLASS_ID") == null ? "" : rowObj
						.get("CLASS_ID").toString());
				classDetails.setDesc(rowObj.get("CLASS_NAME") == null ? ""
						: rowObj.get("CLASS_NAME").toString());
	...
	/**
	 * This method is used to get the Existing Group Attribute Details for Split Color from Database.
	 * 
	 * @param groupId
	 * @return groupAttributeFormList
	 */
	public List<GroupAttributeForm> getExistSplitColorDetails(final String groupId) {
		...
		try {
			...
			query.setParameter("styleIdSql", groupId);
			query.setFetchSize(100);
	...
	/**
	 * This method is used to get the Existing Group Attribute Details for Split Sku from Database.
	 * 
	 * @param groupId
	 * @return groupAttributeFormList
	 */
