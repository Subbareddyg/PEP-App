...
GoogieSpell.prototype.decorateTextarea = function(id) {
    ...
    if(this.text_area != null) {
        if(!AJS.isDefined(this.spell_container) && this.decoration) {
            var table = AJS.TABLE();
            AJS.setClass(table,"spellCheckLink");
            var tbody = AJS.TBODY();
            var tr = AJS.TR();
			...
			AJS.setWidth(table, spell_container, 125);
...
GoogieSpell.createXMLReq = function (text) {
    return '<?xml version="1.0" encoding="utf-8" ?><spellrequest textalreadyclipped="0" ignoredups="0" ignoredigits="1" ignoreallcaps="0"><text>' + text + '</text></spellrequest>';
}
