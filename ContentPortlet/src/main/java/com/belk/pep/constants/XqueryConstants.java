...
public class XqueryConstants {
    ...
    public String getGlobalAttributesQuery(String orinNumber){
        ...
        String GET_GLOBAL_ATTRIBUTES_XQUERY = " WITH Input(ORIN) AS "
        		...
        		+" XMLTable( 'for $i in $item/pim_entry/entry/*" 		
 				...
 				+" return     " 
                +" $i' passing pet.XML_DATA AS \"item\""
        		+" columns OmniChannelBrand VARCHAR2(50) path 'OmniChannelBrand'," 
        		...
        		+" Channel_Exclusive VARCHAR2(30) path 'Channel_Exclusive', "
        		...
        		+" XMLCAST( (XMLQUERY('pim_entry/entry/Item_Ctg_Spec/Description/Long' PASSING  item.xml_data RETURNING CONTENT ) ) AS VARCHAR2(1000) ) Long_Description,"
        		+" pet.Belk_Exclusive,"
        		+" pet.Channel_Exclusive,"
        		+" XMLCAST( (XMLQUERY('pim_entry/entry/Item_Ctg_Spec/Supplier[Primary_Flag eq \"true\"]/ Direct_Ship_Flag' PASSING  item.xml_data RETURNING CONTENT ) ) AS VARCHAR2(1000) ) SDF,"
        		+" pet.BOPIS,"
    ...
    public  String getStyleAndStyleColorAndSKU(String roleName, String orinNumber)
    {
        ...
        String GET_STYLE_STYLECOLOR_SKU_INFORMATION_XQUERY = " WITH INPUT( ORIN) AS " 
            ...
            +"      VENDOR_SIZE_CODE,                                              "
            +"      VENDOR_SIZE_DESC,                                              "
            +"      aic.omnisizecode OMNI_SIZE_CODE,                               "
            +"      pet.pet_state PETSTATUS,                                       "
            ...
            +"      VENDOR_SIZE_CODE                                                                     "
            +"      || ' '                                                                               "
            +"      || VENDOR_SIZE_DESC Vendor_Size, VENDOR_SIZE_CODE,                                   "
            +"      OMNI_SIZE_CODE,                                                                      "
            +"      PETSTATUS,                                                                           "
            +"      CONTENTSTATUS,                                                                       "
            +"      completion_date,                                                                     "
            ...
            +"        FROM Items i                                                                       "
            +"        WHERE i.Entry_Type IN ('SKU', 'Style', 'StyleColor', 'Complex Pack', 'PackColor')                "
            +"      ) ,                                                                                         "
            +"      TypeIndex typInd                                                                            "
            ...
            +"    F.dept_id, F.class_id, F.VENDOR_SIZE_CODE                                                     "
            +"  FROM Final F                                                                                    "
