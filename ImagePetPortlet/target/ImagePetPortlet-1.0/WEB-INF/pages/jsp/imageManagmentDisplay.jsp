		         ...
		         data: { selectedColorOrin:selectedOrin },
					...
					if(val.imageStatus == 'Initiated' || val.imageStatus == 'Rejected'){						
		             ...
		             if (typeof val.imageID  !== "undefined" && val.imageID){
						...
						VPISampleImageRows(val.imageID,val.imageName,val.imageLocation,val.shotType ,val.linkStatus,val.imageStatus,val.sampleId,val.sampleReceived,val.silhouette,val.turnInDate,val.sampleCordinatorNote,val.action,val.role, shotTypeParamArray);
...
function removeVPISampleImageRows(imageId,imageName,rowCount,selectedOrin){  
  ...
  var table = document.getElementById("vImage"); 
  //alert(document.getElementById("removeFlagOff").value);
  inputChangedNew = document.getElementById("removeFlagOff").value;
  //alert('inputChangedNew:::' +inputChangedNew);
...
silhouette,turnInDate,sampleCordinatorNote,action,role, shotTypeParamArray) {	
	...
	if(imageStatus  == "Initiated"){
		...
		getValuesforSubmitorRejectAjax(imageId,imageStatus,element7);
		return false;
	};
}else{
		var cell6 = row.insertCell(5);	
	...
	if(typeof imageStatus  == "undefined" || imageStatus  == "Initiated"){
		...
		element12.onclick = function(){
			getValuesforSubmitorRejectAjax(imageId,imageStatus,element12);
			return false;		
		};
	}else{
		var cell11 = row.insertCell(5);	
	...
	if(imageStatus  == 'Approved' || imageStatus  == 'Rejected' || imageStatus  == 'Rejected_By_DCA' || imageStatus  == 'Completed'){
		...
		element13.onclick = function(){
			getValuesforSubmitorRejectAjax(imageId,imageStatus,element13);
			return false;			
		};	
	}	
	...
	if(imageStatus  == 'Completed'){
		...
		cell13.innerHTML = '<a href="javascript:confirmRemovePopUp('+imageId+',\''+imageName+'\','+rowCount+')">Remove</a>';
	}
...
function getValuesforSubmitorRejectAjax(imageId,imageStatus,element){
...
<input type="hidden" id="saveImageShotTypeId" name="saveImageShotTypeId" value="${saveImageShotType}" />
		...
		<fmt:message key="label.imageManagement.header"/>				
				...
				<!-- Table Grid logic Start -->
						...
						<c:forEach items="${workFlow.styleColor}" var="style"
								...
								<tr name="treegrid2"
									class="treegrid-${subcount} treegrid-parent-${countList}"  onclick="setUploadVPILink('${getUploadVPIDetails}','<c:out value="${style.orinNumber}" />','${removeSampleImageUrl}');checkApproveImageStatus('${style.orinNumber}','${style.imageStatus}');disableAllButtons();">
				...
				//alert('RowHeight::' + document.getElementById('petTableRowId').offsetHeight); 
