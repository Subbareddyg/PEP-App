...
<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/createPet.js"></script>


<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/jquery.treegrid.js"></script>
<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/jquery.min.js"></script>
<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/treegridlocal.js"></script>
...
window.location = '<%=request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+"/wps/portal/home/worklistDisplay" %>';
	...
	<c:if test="${not empty errorMsg}">
		...
		<img src="<%=response.encodeURL(request.getContextPath()+"/img/iconWarning.gif")%>" alt="" class="icon" id="ext-gen28">
		<c:out value="${errorMsg}" /></li>
	</c:if>
	
	<c:if test="${not empty responseMsg}">

	<c:out value="${responseMsg}" /></li>
	</c:if>
...
<form:form commandName="workflowForm" method="post"
			action="${formAction}">
...
<ol>	
	<li>
		...
		<input type="text" name="orinNumber"  value="${orinNumber}" maxlength="12" />
...
<th><fmt:message key="createpet.label.size"/></th>
			...
			<c:forEach items="${workflowForm.workFlowList}" var="workFlow"
				varStatus="status">
					...
					<td align="center" style="width: 54px;  height: 24px;"><input type="checkbox" name="selectedItems"
						value="<c:out  value="${workFlow.orin} "/>" /></td>
				<td><c:out value="${workFlow.orin}" />
				<input type="hidden" name="StyleLOrin"  value="${workFlow.orin} "/>
				</td>

					<td><c:out value="${workFlow.orinDesc}" />
					</td>

					<td><c:out value="${workFlow.supplierSiteName}" />
					</td>
					<td><c:out value="${workFlow.colorDes}" />
					</td>
					<td>
					</td>					
				</tr>
			<c:if test="${workFlow.styleList != null}">	
			<c:forEach items="${workFlow.styleList}" var="style"
					...
					<tr name="treegrid2"
						...
						<td>
						<input  style="margin-left: 1.1cm;" type="checkbox" name="selectedStyles"
							value="<c:out  value="${style.orin}, ${style.colorDes} "/>" /></td>
						<td><c:out value="${style.orin}" /></td>
						<td><c:out value="${style.orinDesc}" />
						</td>

						<td><c:out value="${style.supplierSiteId}" />
						<c:out value="${style.supplierSiteName}" />
						</td>
						<td><c:out value="${style.colorCode}" /> <c:out value="${style.colorDes}" />
						</td>
						<td><c:out value="${style.sizeDesc}" />
						</td>						
					</tr>	
				...
				<c:forEach items="${style.skuList}" var="sku"
					varStatus="Skustatus">
					...
					<tr name="treegrid2"
						class="treegrid-${subcount} treegrid-parent-${countList}">
						<td>
					<input  style="margin-left: 1.9cm;" type="checkbox" name="selectedSkus"
							value="<c:out  value="${sku.orin} "/>" /></td>
						<td><c:out value="${sku.orin}" /></td>
						<td><c:out value="${sku.orinDesc}" />
						</td>

						<td><c:out value="${sku.supplierSiteId}" />
						<c:out value="${sku.supplierSiteName}" />
						</td>
						<td><c:out value="${sku.colorCode}" />  <c:out value="${sku.colorDes}" />
						</td>
						<td><c:out value="${sku.sizeCode}" />    <c:out value="${sku.sizeDesc}" />
						</td>						
					</tr>	
