				...
				<!-- Table Grid logic Start -->
						...
						<c:forEach items="${workflowForm.workFlowlist}" var="workFlow"
							varStatus="status">
								...
								<td align="center"><input type="checkbox" name="styleItem" id="styleItem" class="checkbox1"
									value="${workFlow.orinNumber}_${workFlow.petStatus}" onclick="childCheckedRows(this,'${workFlow.orinNumber}');getPetStatValue('${workFlow.petStatus}')" /></td>
						...
						<c:forEach items="${workFlow.styleColor}" var="style"
								...
								<tr name="treegrid2"
									...
									<td><input type="checkbox" parentOrinNo="${workFlow.orinNumber}" name="selectedStyles" class="checkbox1"
										value="${style.orinNumber}_${style.petStatus}" onclick="getStyleColorPetStatValue('${style.petStatus}')" /></td>
									...
									<c:choose>
										<c:when test="${workflowForm.roleEditable =='yes' }" >
										 <a href="#" onclick="contentStatus('<c:out value="${style.contentStatus}" />','<c:out value="${workFlow.orinNumber}" />')"><c:out value="${style.contentStatus}" /></a>
										</c:when>
										<c:when test="${workflowForm.readOnlyUser =='yes' }" >	
										   <a href="#" onclick="contentStatus('<c:out value="${style.contentStatus}"/>','<c:out value="${workFlow.orinNumber}"/>')"><c:out value="${style.contentStatus}"/></a>
										</c:when>
										<c:otherwise>
										<c:out value="${style.contentStatus}" />
										</c:otherwise>
									</c:choose>
									</td>
									<td>
									<c:choose>
										<c:when test="${workflowForm.roleEditable =='yes' }" >
										 <a href="#" onclick="imageStatus('<c:out value="${style.imageStatus}" />','<c:out value="${workFlow.orinNumber}" />','${style.petStatus}')"><c:out value="${style.imageStatus}" /></a>
										</c:when>
								...
								<td>
									<input type = "hidden" id="hiddenRolName" value ="${workflowForm.roleName}" />
									<c:choose>
										...
										<input type="text" id="tCompletionDate" name="tCompletionDate" value='${style.completionDate}'  onchange="checkcompletiondate(this,'${subcount}')"/>
										<input type="hidden" id="tbCompletionDate${subcount}" name="tbCompletionDate${subcount}" value='${style.completionDate}' class = '${workFlow.orinNumber}_child '/>										 
										<input type="hidden" id="tbOrinNumber${subcount}" name="tbOrinNumber${subcount}" value='${workFlow.orinNumber}'/>
										<input type="hidden" id="tbStyleOrinNumber${subcount}" name="tbStyleOrinNumber${subcount}" value='${style.orinNumber}'/>
										</c:when>
										<c:otherwise>										
															
											<c:if test = "${style.petStatus ne 'Deactivated'}">										
												<c:out value="${style.completionDate}" />
												<input type="hidden" id="tbCompletionDate${subcount}" name="tbCompletionDate${subcount}" value='${style.completionDate}' class = '${workFlow.orinNumber}_child '/>
											</c:if>	
									...
									<td style="display: none;"><c:out value="${style.petStatus}" />
								</td>
								</tr>
							</c:forEach>
				...
				<div id="search_tabs">
					<div id="login1" title="" class="tab">
						<li align="right">
						     ...
						     <input type="button" value="<fmt:message key="worklist.adv.jsp.main.close.button.label"/>" onclick='searchClose();$("#dialog_ASearch").hide();$("#overlay_Image_advSearch").hide();' value=""/>	
